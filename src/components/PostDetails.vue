      ...
      deleteComment(commentId)
        .then(({ data }) => {
          if (data === 1) {
            ...
            console.warn("Comment deletion might not be fully successful. API response:", data);
...
<template>
  ...
  <div v-else-if="currentPost" class="content">
    ...
    <div class="buttons">
      <button class="button is-link" @click="$store.commit('setInSidebar', 'updatingPost')">Edit Post</button>
      </div>
    ...
    <AddComment
      ...
      @updateCommentsList="addNewComment"
    />
  </template>
